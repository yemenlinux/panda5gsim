...
class PointActor(DirectObject):
    def __init__(self, **kwargs):
        ...
        # self.Actor.setPos(self.position)
        # set tags
        if self.type == "UE" or self.type == 'UT':
            if not hasattr(self, 'device_type'):
            ...
            self.device.setZ(self.Actor, 1.5)
            color = (0,0,1,1)
        elif self.type == "UB":
            if not hasattr(self, 'device_type'):
            ...
            self.device.reparentTo(self.Actor)
            color = (0,0,1,1)
        elif self.type == 'GB' or self.type == 'BS':
            if not hasattr(self, 'device_type'):
                self.device_type = 'gBS'
            self.Actor.setTag('type', 'ground_bs')
            # self.Actor.setTag('actor', 'ground_actor')
            self.device = Device(self.device_type, mimo = (1,1))
            self.device.reparentTo(self.Actor)
            # set height of device to 1.5 m above ground
        ...
        # set color of device to blue
        pointnode = PointNode(f'p_{self.name}')
        pointnode.reparentTo(self.Actor)
        pointnode.setPos(self.Actor, self.device.getPos(self.Actor))
        ...
        # # tasks
        # self.taskMgr.add(self.set_device_direction, 'device_direction')
